function is_connected(g::UndirectedMultigraph)
    simple_edges = Set(Tuple(e) for e in edges(g))
...
function arboreal_pair(F1::UndirectedMultigraph, F2::UndirectedMultigraph)
...
function arboreal_pair_experiment(n_vertices::Int, graph_indices::Vector{Int} = collect(1:n_laman_graphs(n_vertices)))
    LGD = include("LamanGraphDatabase/LamanGraphs"*string(n_vertices)*".txt")
